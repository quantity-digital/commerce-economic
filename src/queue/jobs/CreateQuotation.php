...
class CreateQuotation extends BaseJob
{
	/**
	 * @var int Order ID
	 */
	...
	public function execute($queue)
	{
		try {
			...
			$response = Economic::getInstance()->getQuoations()->createQuotationFromOrder($order);
			...
			if ($response) {
				$quotation = $response->asObject();
				$this->setProgress($queue, 0.91);
				$order->draftInvoiceNumber = (int) $quotation->draftInvoiceNumber;
